...
public interface CheckOutRepository extends JpaRepository<CheckOut, Long> {
    ...
    @Query("""
            ...
            """)
    List<Instant> getCheckOutDatesOfAttendanceIds(List<Long> attendanceIds);
    ...
    @Query("""
            ...
            WHERE co.attendance.id = ?1
            """)
    List<Instant> getCheckOutDatesOfAttendanceId(Long attendanceId);

    @Query("""
            SELECT MAX(c.date) FROM CheckOut c WHERE c.attendance.id = ?1
            
            """)
    Instant getLastCheckOutDateOfAttendanceId(Long attendanceId);
}
